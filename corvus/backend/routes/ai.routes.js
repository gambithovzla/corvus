...
const { getSystemPrompt } = require('../voiceProfiles');
...
// Genera contenido usando Claude con Voice Profiles
router.post('/generate', async (req, res) => {
  ...
  try {
    ...
    if (error.status === 401) {
      return res.status(401).json({ error: 'API Key de Anthropic inválida' });
    }
    if (error.status === 429) {
      return res.status(429).json({ error: 'Límite de API alcanzado. Espera un momento.' });
    }
